...
$(document).ready(function () {
  ...
  if ($(window).width() < 768) {
    ...
    $(".download-parent .cta-mob").attr("data-aos-delay", "700");
    $(".features .mobile-hand-img").attr("data-aos-delay", "400");
  }
  ...
  if ($("html").attr("dir") == "ltr") {
    ...
    $(".features .mobile-hand-img").attr("data-aos", "fade-left");
  }
  ...
  $(window).on("scroll", function () {
    if ($(window).scrollTop() > 1) {
      $("header").addClass("blur-fixed-header");
    }
    if ($(window).scrollTop() == 0) {
      $("header").removeClass("blur-fixed-header");
    }
  });
